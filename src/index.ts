...
exports.handler = async (event: any, context: LambdaType.Context) => {
    ...
    for(let screenName of configOnDb.screenNames) {
        ...
        for(let tweet of tweets) {
            if (maxId < tweet.id_str && maxId.length <= tweet.id_str.length) { maxId = tweet.id_str; }
            for(let keyword of configOnDb.keywords) {
    ...
    if (founds.length > 0) {
        ...
        const text = [
            ...
            `現在の監視アカウント: ${accounts}`,
            `現在の監視キーワード: https://${Config.s3.region}.amazonaws.com/${Config.s3.bucket}/${Config.s3.keyName}`].join("\n");
        await twitterClient.sendTweet(text);

        for (let found of founds) {
            await twitterClient.sendTweet(`@null 理由: "${found.keyword}"にマッチしました\n${TwitterClient.tweetToStatusUrl(found.tweet)}`);
        }
    }
